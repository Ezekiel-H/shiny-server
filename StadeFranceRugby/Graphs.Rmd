...
top4$Defensive_Tackles[top4$Defensive_Tackles == 0] <- NA

top4 <- top4 %>% summarise(Defensive_Tackles = mean(Defensive_Tackles, na.rm=TRUE)) %>% mutate(Clubs = "Top4")


stade <- stats %>% group_by(Week) %>% filter(Clubs == "Paris") %>% summarise(Defensive_Tackles = mean(Defensive_Tackles, na.rm=TRUE)) %>% mutate(Clubs = "Stade")
