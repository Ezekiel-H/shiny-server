...
libs <- c("tidyverse", "lubridate", "httr", "rea", "dplyr", "shiny", "rmarkdown", "knitr", "tibble", "highcharter", "ggplot2", "readr") #,

lapply(libs, require, character.only = TRUE)

setwd("C:\\Users\\Ezekiel\\Desktop\\R\\StadeFranceRugby")
...
top4$Plaquages.réussis....[top4$Plaquages.réussis.... == 0] <- NA

top4 <- top4 %>% summarise(Tackles = mean(Plaquages.réussis...., na.rm=TRUE)) %>% mutate(Clubs = "Top4")
                                                                                        
                                                                                                                          
stade <- stats %>% group_by(Week) %>% filter(Clubs == "Paris") %>% summarise(Tackles = mean(Plaquages.réussis...., na.rm=TRUE)) %>% mutate(Clubs = "Stade")
...
top4$Plaquages.défensifs.réussis[top4$Plaquages.défensifs.réussis == 0] <- NA

top4 <- top4 %>% summarise(Defensive_Tackles = mean(Plaquages.défensifs.réussis, na.rm=TRUE)) %>% mutate(Clubs = "Top4")


stade <- stats %>% group_by(Week) %>% filter(Clubs == "Paris") %>% summarise(Defensive_Tackles = mean(Plaquages.défensifs.réussis, na.rm=TRUE)) %>% mutate(Clubs = "Stade")
